...
using namespace Nexus::Importer::glTF;

SampFilter GetFilterMode(int32_t filter)
{
	switch (filter) 
	{
	case 9728:
		return SampFilter::Nearest;
	case 9729:
		return SampFilter::Linear;
	case 9984:
		return SampFilter::Nearest;
	case 9985:
		return SampFilter::Nearest;
...
SampWrapMode GetWrapMode(int32_t mode)
{
	switch (mode)
	{
	case 10497:
		return SampWrapMode::Repeat;
	case 33071:
		return SampWrapMode::Clamped_To_Edge;
	case 33648:
		return SampWrapMode::Mirrored_Repeat;
	default:
...
void CalculateTangentAndBiTangent(VertexData& v0, VertexData& v1, VertexData& v2, bool foundTangent, bool foundBiTangent)
{
	glm::vec3 deltaPos1 = v1.position - v0.position;
	glm::vec3 deltaPos2 = v2.position - v0.position;
...
bool Nexus::Importer::glTF::Load(const std::filesystem::path& filepath, MeshSpecifications* specs)
{
	tinygltf::Model scene;
	tinygltf::TinyGLTF Importer;
