...
Nexus::VulkanSwapchain* Nexus::VulkanSwapchain::s_Instance = nullptr;

Nexus::VulkanSwapchain::VulkanSwapchain(Window* window)
{
	s_Instance = this;
	...
	// Swapchain Format, Mode and Extent
	{
		...
		NEXUS_LOG("Vulkan", "Min Image Count: %i", m_MinImageCount);
...
void Nexus::VulkanSwapchain::Initialize()
{
	{
	...
	// Swapchain
	{
		...
		NEXUS_LOG("Vulkan", "Swapchain Created");
	}
	...
	// Swapchain Images
	{
		...
		NEXUS_LOG("Vulkan", "Swapchain Images Acquired: %i", count);
		...
		NEXUS_LOG("Vulkan", "Swapchain Image Attachment Created : %i", count);
	}
}

void Nexus::VulkanSwapchain::Shutdown()
{
	for (uint32_t i = 0; i < m_SwapchainImages.size(); i++)
	{
		vkDestroyImageView(m_device, m_SwapchainImages[i].view, nullptr);
	}
	NEXUS_LOG("Vulkan", "Swapchain Image Attachment Destroyed : %i", m_SwapchainImages.size());

	vkDestroySwapchainKHR(m_device, m_Swapchain, nullptr);
	NEXUS_LOG("Vulkan", "Swapchain Destroyed");
