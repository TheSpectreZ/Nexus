...
Nexus::VulkanPhysicalDevice::VulkanPhysicalDevice(const std::vector<const char*>& requiredExtensions)
{
	...
	if (!m_physicalDevice)
	{
		NEXUS_LOG_TRACE("Vulkan Discrete Physical Device Not Found");
		m_physicalDevice = devices.back();
	}

	vkGetPhysicalDeviceProperties(m_physicalDevice, &m_properties);

	NEXUS_LOG_INFO("Vulkan Physical Device Acquired: {0}", m_properties.deviceName);
...
Nexus::VulkanDevice::VulkanDevice(Ref<VulkanPhysicalDevice> device,const std::vector<const char*>& requiredExtensions, const std::vector<const char*>& requiredLayers)
{
	...
	NEXUS_LOG_INFO("Vulkan Logical Device Created");
	...
	// Queues
	{
		...
		NEXUS_LOG_WARN("Vulkan {0} Queue Created, Family Index: {1}", "Graphics", m_QueueFamilyIndices.Graphics);
		NEXUS_LOG_WARN("Vulkan {0} Queue Created, Family Index: {1}", "Present ", m_QueueFamilyIndices.Present);
		NEXUS_LOG_WARN("Vulkan {0} Queue Created, Family Index: {1}", "Transfer", m_QueueFamilyIndices.Transfer);
		NEXUS_LOG_WARN("Vulkan {0} Queue Created, Family Index: {1}", "Compute ", m_QueueFamilyIndices.Compute);
	}
	...
	// Vulkan Memory Allocation
	{
		...
		NEXUS_LOG_INFO("Vulkan Allocator Created");
	}
} 
...
Nexus::VulkanDevice::~VulkanDevice()
{
	...
	NEXUS_LOG_INFO("Vulkan Allocator Destroyed");

	vkDestroyDevice(m_logicalDevice, nullptr);
	NEXUS_LOG_INFO("Vulkan Logical Device Destroyed");
}
